...
def test(metric, original_paper, generated_review) -> float:
...
for file_path in json_files:
    ...
    try:
        with open(file_path, "r") as f:
            ...
            # raw papers version
            paper_map = {p.pdf_name for p in parsed_papers}

            all_raw_papers = "\n".join(paper_map[proc.paper_name] for proc in result_pipeline.processed_paper)
            ...
            scores["SOTA"] = test(research_gap_metric,all_raw_papers, SOTA_generated)
            scores["final_summary"] = test(synthesis_quality_metric,all_raw_papers, final_summary_generated)
